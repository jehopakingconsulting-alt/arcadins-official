  ...
  return (
    <section aria-labelledby="stats-title" className="rounded-xl border border-[color:var(--border-gold)] bg-white p-5">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-4">
        <div>
          <dt className="text-xs text-[#5a6a82]">Temps d&apos;étude</dt>
          <dd className="text-lg font-semibold text-[color:var(--color-navy)]">{hours}h {mins}min</dd>
        </div>
        <div>
          <dt className="text-xs text-[#5a6a82]">Score moyen</dt>
        ...
        <ul className="mt-4 space-y-2">
          {competencies.map((c) => (
            <li key={c.code} className="text-sm">
              <div className="flex justify-between text-xs text-[#5a6a82]"><span>{c.labelKey}</span><span>{c.percent}%</span></div>
