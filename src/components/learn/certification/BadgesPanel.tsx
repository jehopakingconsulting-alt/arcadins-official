    ...
    <section aria-labelledby="badges-title" className="rounded-xl border border-[color:var(--border-gold)] bg-white p-5">
      ...
      {badges.length === 0 ? (
        ...
        <ul className="mt-3 grid gap-3 sm:grid-cols-2">
          {badges.map((b) => (
            <li key={b.badgeDefinitionId} className={`rounded-lg border p-3 ${b.obtained ? "border-[color:var(--color-gold)] bg-[color:var(--color-gold-pale)]/30" : "border-[color:var(--border-gold)] opacity-70"}`}>
              <div className="flex items-center gap-2">
                <span aria-hidden="true">{b.obtained ? "🏅" : "🔒"}</span>
                <p className="font-semibold text-[color:var(--color-navy)]">{b.titleKey}</p>
              </div>
              <p className="mt-1 text-xs text-[color:var(--color-muted)]">{b.descriptionKey}</p>
              <p className="mt-1 text-xs text-[color:var(--color-muted)]">Critère : {b.criteriaKey}</p>
              {b.obtained ? (
