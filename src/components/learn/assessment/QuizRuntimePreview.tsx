"use client";
/**
 * Wrapper de PREVIEW du quiz runtime (Sprint K3A). Monte le `RuntimeProvider` (K2A) sur un runtime
 * déterministe, câble la PROGRESSION : à la soumission, le contrôleur dispatch `QUIZ_SUBMITTED` vers CE
 * runtime → la progression se recalcule (moteurs Sprints A + K2A) et l'affichage se met à jour.
 * Données FICTIVES, horloge fixe, aucune API, aucune donnée réelle, aucun certificat/badge/examen.
 */
...
import { buildModuleQuizDefinition, buildQuizBank, makeFormativeQuizPolicy } from "@/lib/runtime/ui/assessment/quiz-config";
...
/**
 * Variante DÉTERMINISTE (test navigateur uniquement) : banque synthétique dont la bonne réponse connue est la
 * PREMIÈRE option ("o0"). Permet de prouver un résultat RÉUSSI/ÉCHOUÉ en navigateur. Les bonnes réponses
 * restent dans la CLÔTURE du contrôleur (jamais DOM/props/session).
 */
...
export function QuizRuntimePreview({ curriculum, variant = "real" }: { curriculum: ProgramCurriculumV2; variant?: "real" | "det" }) {
  ...
  const makeOptions = useCallback(
    (): QuizControllerOptions => ({
      definition:
        variant === "det"
          ? { id: "quiz-det", programId: curriculum.slug, moduleId: MODULE_ID, version: 1, selection: { count: 3, moduleId: MODULE_ID }, lessonId: lessonRef }
          : buildModuleQuizDefinition({ programSlug: curriculum.slug, moduleId: MODULE_ID, count: 5, lessonId: lessonRef }),
      bank: variant === "det" ? buildDeterministicBank() : buildQuizBank(),
    ...
    [runtime, curriculum.slug, lessonRef, variant],
  );
